...
static bool block_is_spawned(struct block *block) { return block->pid > 0; }
...
static int block_send_key(const char *key, const char *value, void *data) {
  ...
  dprintf(block->in.p.fd_write, ",\"%s\":%s", key, value);
...
static int block_child_stdin(struct block *block) {
  ...
  if (block->interval == INTERVAL_PERSIST) {
    err = sys_close(block->in.p.fd_write);
    if (err)
      return err;
  } else {
    err = sys_open("/dev/null", &block->in.p.fd_read);
    if (err)
      return err;
  }

  err = sys_dup(block->in.p.fd_read, STDIN_FILENO);
  if (err)
    return err;

  return sys_close(block->in.p.fd_read);
}
...
static int block_child_stdout(struct block *block) {
  ...
  err = sys_dup(block->out.p.fd_write, STDOUT_FILENO);
  if (err)
    return err;

  return sys_close(block->out.p.fd_write);
}
...
static int block_fork(struct block *block) {
  ...
  if (block->pid == 0) {
    ...
    // no need to return because block_child ends up with execl call
  }
...
static int block_open(struct block *block) {
  ...
  err = sys_pipe(block->out.fds);
  if (err)
    return err;

  if (block->interval == INTERVAL_PERSIST)
    return sys_pipe(block->in.fds);
...
int block_reap(struct block *block) {
  ...
  case EXIT_URGENT:
    break;
  case 126:
    block_error(block, "Command '%s' not executable", block->command);
    break;
  case 127:
